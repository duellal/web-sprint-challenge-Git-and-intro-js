...
const changeArtist = function(array, index, incorrectKey, correctName){
  array[index][incorrectKey] = correctName;
  return array
}

console.log(changeArtist(artists, 8, 'name', 'Vincent Van Gogh')[8])
...
function getArtistByIndex(array, index) {
  for (let i = 0; i < array.length; i++) {
    if (array[i].id === index) {
      return `the artist at index ${index} is ${array[i].name}`
    }
  }
}
...
function listOfNames(array) {
  ...
  for(let i=0; i < artistNames.length; i++){
    artistNames.splice(i, 1, artistNames[i].name)
  }
  return artistNames
}
...
function removeArtist(array, index) {
  ...
  for (let i=0; i < removedArtist.length; i++){
    if(removedArtist[i].id === index){
      removedArtist.splice(i, 1)
    }
  }
